...
enum categoryEnum : logCategory::category_type
{
	...
	forDummy = MAKE_LOG_CATEGORY_VALUE(1),//ダミー　※ログ出力／画面通知なし
	forNotice = MAKE_LOG_CATEGORY_VALUE(2),//画面通知専用 ※ログ出力なし
	forFileSystem = MAKE_LOG_CATEGORY_VALUE(3),//ファイルシステム関係
	forResource = MAKE_LOG_CATEGORY_VALUE(4),//リソース関係
	for3D = MAKE_LOG_CATEGORY_VALUE(5),//3Dグラフィックス関係
	for2D = MAKE_LOG_CATEGORY_VALUE(6),//2Dグラフィックス関係
	forSound = MAKE_LOG_CATEGORY_VALUE(7),//サウンド関係
	//ログレベル／画面通知レベル変更用
