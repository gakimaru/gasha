...
//名前付きデータ参照
//※データ参照に対して、名前を付けて管理。
//※名前付きデータ参照（テーブル）自体はシングルトンとして振る舞う。
//※名前と型さえ分かっていれば、実体の extern や include なしに、どこからでも参照可能。
//※値の取得／変更以外に、演算に対応。加算／減算／乗算は、ラップアラウンド演算と飽和演算に対応。
//※設定により、データごとの共有ロック／排他ロックが可能。
//※操作用構造体を複数用意することにより、複数のインスタンスを作成可能。ロック制御の有無はインスタンスごとに設定。
//※データの列挙が可能。
//※デバッグ機能の作成に便利。デバッグメニューからデバッグモードを切り替えたり、特定の情報をデバッグウインドウでモニターするなど。
//※スクリプトの処理に便利。スクリプト用の関数を一つ一つ作成せずに、名前付きデータ参照を透過的に利用することで生産性を向上できる。
...
//名前付きデータ参照操作用テンプレート構造体
template<class OPE_TYPE, std::size_t _TABLE_SIZE>
using namedRef_baseOpe = named_ref::baseOpe<OPE_TYPE, _TABLE_SIZE>;

//名前付きデータ参照
template<class OPE_TYPE>
using namedRef = named_ref::table<OPE_TYPE>;
