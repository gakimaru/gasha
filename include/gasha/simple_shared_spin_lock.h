...
public:
	...
	inline GASHA_ unique_shared_lock<simpleSharedSpinLock> lockUnique(const GASHA_ try_to_lock_t&);
	inline GASHA_ unique_shared_lock<simpleSharedSpinLock> lockUnique(const GASHA_ try_to_lock_shared_t&);
	...
	//※排他ロック（ライトロック）取得を伴う
	inline GASHA_ lock_guard<simpleSharedSpinLock> lockScoped();
	//排他ロック（ライトロック）取得を試行
	...
	//※共有ロック（リードロック）取得を伴う
	inline GASHA_ shared_lock_guard<simpleSharedSpinLock> lockSharedScoped();
	//共有ロック（リードロック）取得を試行
	//※取得に成功した場合、trueが返るので、ロックを解放する必要がある
