...
namespace rb_tree
{
	...
	inline typename stack_t<OPE_TYPE>::info_t* stack_t<OPE_TYPE>::push(const typename OPE_TYPE::node_type* node, const bool is_large)
	{
	...
	//スタックからノード情報を取得
	template<class OPE_TYPE>
	inline typename stack_t<OPE_TYPE>::info_t* stack_t<OPE_TYPE>::pop()
	{
		if (m_depth == 0)
			return nullptr;
	...
	//※要素が減らない
	template<class OPE_TYPE>
	inline typename stack_t<OPE_TYPE>::info_t* stack_t<OPE_TYPE>::top()
	{
		if (m_depth == 0)
			return nullptr;
	...
	//スタックの現在の深さを更新
	template<class OPE_TYPE>
	inline void stack_t<OPE_TYPE>::setDepth(const int depth)
	{
		if (depth < 0)
			m_depth = 0;
	...
	//スタックの現在の深さをリセット
	template<class OPE_TYPE>
	inline void stack_t<OPE_TYPE>::reset()
	{
		m_depth = 0;
	}
	//スタックの現在の幅を算出
	...
	//※小側を-1、大側を+1として計算
	template<class OPE_TYPE>
	inline std::int64_t stack_t<OPE_TYPE>::calcBreadth()
	{
	...
	//ムーブオペレータ
	template<class OPE_TYPE>
	inline stack_t<OPE_TYPE>& stack_t<OPE_TYPE>::operator=(const stack_t<OPE_TYPE>&& rhs)
	{
		m_depth = rhs.m_depth;
		if (m_depth > 0)
	...
	//ムーブコンストラクタ
	template<class OPE_TYPE>
	inline stack_t<OPE_TYPE>::stack_t(const stack_t<OPE_TYPE>&& obj) :
		m_depth(obj.m_depth)
	{
		if (m_depth > 0)
	...
	template<class OPE_TYPE>
	inline container<OPE_TYPE>::iterator::iterator(stack_type&& stack, const container& con, typename container<OPE_TYPE>::value_type* value, const bool is_end) :
	...
	template<class OPE_TYPE>
	inline container<OPE_TYPE>::reverse_iterator::reverse_iterator(stack_type&& stack, const container& con, typename container<OPE_TYPE>::value_type* value, const bool is_end) :
		m_stack(std::move(stack)),
		m_con(&con),
		m_value(value),
		m_isEnd(is_end)
