...
public:
	//アクセッサ
	const char* name() const { return "lfDualStackAllocator"; }//アロケータ名
	...
	inline allocationOrder_t allocationOrder() const { return m_allocateOrder.load(); }//現在のアロケート方向
	inline void setAllocateOrder(const allocationOrder_t order){ m_allocateOrder.store(order); }//現在のアロケート方向を変更
...
private:
	...
	std::atomic<allocationOrder_t> m_allocateOrder;//現在のアロケート方向
};
