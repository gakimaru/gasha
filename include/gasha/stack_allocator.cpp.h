...
void* stackAllocator<LOCK_TYPE, AUTO_CLEAR>::alloc(const std::size_t size, const std::size_t align)
{
	...
	//サイズとアラインメントをチェック
	char* now_ptr = m_buffRef + m_size;
	char* new_ptr = adjustAlign(now_ptr, align);
	const std::size_t padding_size = new_ptr - now_ptr;
	const size_type new_size = static_cast<size_type>(padding_size + size);
	if (m_size + new_size > m_maxSize)
	{
