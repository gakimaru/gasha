#pragma once
#ifndef GASHA_INCLUDED_RADIX_SORT_INL
#define GASHA_INCLUDED_RADIX_SORT_INL

//--------------------------------------------------------------------------------
// 【テンプレートライブラリ】
// radix_sort.inl
// 基数ソート【インライン関数／テンプレート関数定義部】
//
...
#include <gasha/radix_sort.h>//基数ソート【宣言部】
...
inline std::size_t radixSort(T* array, const std::size_t size, GET_KEY_FUNCTOR get_key_functor)
{
	...
	key_t* key_tbl = new key_t[size];//※newでメモリ確保
	{
		...
		for (std::size_t index = 0; index < size; ++index, ++elem_p, ++key_p)
			key_max = std::max(key_max, key_p->set(elem_p, _private::radix_key<T, GET_KEY_FUNCTOR>::getKey(*elem_p, get_key_functor)));
	}
	//キーの最大値から最大の長さを算出
	std::size_t key_len =
	...
	KEY_TYPE_U key_max = 0;//キーの最大値
	{
		...
		for (index_type index = 0; index < size; ++index, ++elem_p, ++key_p)
			key_max = std::max(key_max, key_p->init(elem_p, _private::radix_key<T, GET_KEY_FUNCTOR>::getKey(*elem_p, get_key_functor)));
	}
	//キーの最大値から最大の長さを算出
...
#endif//GASHA_INCLUDED_RADIX_SORT_INL
